...
class Turtlebot3Ros1DisSafeEnv(gym.Env):
    ...
    def __init__(self):
        ...
        rospy.init_node('Gym_Turtlebot3_Dis_Safe_Env_Node', anonymous=True)
        ...
        ************************************************************"""
        ...
        self.states_sub = rospy.Subscriber("/gazebo/link_states", LinkStates, self.gazebo_callback)
        ...
        ************************************************************"""
        ...
        state, info = self.take_observation(action)
    ...
    *******************************************************************************"""
    ...
    def gazebo_callback(self, msg):
        try:
           ...
           self.link_pose = msg.pose[ind]           
           _, _, self.gazebo_pose_theta = self.euler_from_quaternion(msg.pose[ind].orientation)
        except ValueError:
           pass
    ...
    def take_observation(self, action):
        ...
        for i in range(len(observation)):
            if observation[i] == float('Inf'):
                scan_range.append(round(3.5/3.5, 4))
            elif np.isnan(observation [i]):
                scan_range.append(0)
            elif observation[i] > 3.5:
                scan_range.append(3.5/3.5)
            else:
                scan_range.append(round(observation[i], 3)/3.5)
        
        obs = scan_range + [(goal_angle+math.pi)/(2*math.pi), goal_distance/10.0, action]
        obs = np.array(obs, dtype=np.float32)
        
        info = {'x': round(self.last_pose_x, 3), 'y': round(self.last_pose_y, 3), 'theta': round(self.last_pose_theta, 3), 'gazebo_x' : round(self.link_pose.position.x,3) , 'gazebo_y' : round(self.link_pose.position.y,3), 'gazebo_theta' : round(self.gazebo_pose_theta,3)}
    ...
    def step(self, action):
        ...
        obs, info = self.take_observation(action)
        reward, done = self.setReward(obs)
