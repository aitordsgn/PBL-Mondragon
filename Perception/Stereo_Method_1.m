...
for j = 1:n
    ...
    % Display the adjusted disparity range
    % disp('Adjusted Disparity Range:');
    % disp(disparityRange);
    ...
    transformedPoints3D_homogeneous = (TA * homogInv(camR1_h) * points3D_homogeneous')';
    ...
    % figure;
    % pcshow(tempPtCloud);
    % title('Nube de Puntos 3D Temporal');
    % xlabel('X (mm)');
    % ylabel('Y (mm)');
    % zlabel('Z (mm)');
    ...
    if isempty(ptCloud.Location)
        ...
        %ptCloud = pcdownsample(ptCloud,"nonuniformGridSample",6);
...
save('Reconstructed_PointCloud_Meth1.mat', 'ptCloud');
figure;
pcshow(ptCloud);
axis equal
...
%writeSurfaceMesh(mesh,"Cubo_Mesh_1.stl")
%pcwrite(ptCloud, 'Cubo_PointCloud_1.ply')
%% FUNCTIONS
